...
Status: Complete as of 2026-09-21. The audit combines automated evidence,
applicable M0 physical evidence, and M1 hardware evidence. Unperformed M1-only
physical repetitions are disclosed in `closure-exceptions.md`.
...
| FR-USB-002 | `MainActivity.requestUsbPermission` and attachment receiver | `PhysicalDeviceGateTest` permission-denial regression | M0 clean-install permission flow and M1 Pixel 8a grants recorded; not repeated during the final 32-buffer gate |
...
| FR-USB-006 | `NativeRadioSession.close`; service `finally`/stop/detach paths | `NativeSessionStateTest`; coordinator tests | Detach drained queues and stop removed the foreground service; direct native-handle instrumentation was not added |
...
| FR-LOC-002 | configurable monotonic-age policy | stale-fix unit test | Final preflight displayed stale GPS state; no dedicated physical GPS-gap repetition |
| FR-LOC-003 | bounded interpolation with explicit marker/provider | interpolation unit test | Accepted from deterministic evidence; no dedicated physical GPS-gap repetition |
| FR-LOC-004 | nullable aggregate location plus `MISSING`/`STALE` state | Room unlocated-aggregate test | Final survey preserved 2,340 unlocated observations; no dedicated permission-denial repetition |
...
| NFR-PERF-003 | bounded nonblocking callback path | M0 8 MS/s evidence; queue tests | M0 physically demonstrated 8 MS/s; M1's clean physical gate used 2 MS/s and was not repeated at 8 MS/s |
...
- `./gradlew.bat lint test assembleDebug`, `scripts/Test-ReceiveOnly.ps1`,
  `scripts/Test-NativeExports.ps1`, and `scripts/Test-Planning.ps1`: passed
  2026-09-21 for milestone closure. Connected tests were not repeated; the
  accepted limitation and prior result-file evidence are documented in
  `closure-exceptions.md`.
